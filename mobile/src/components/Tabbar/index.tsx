...
const left = shape.line<{x: number, y: number}>()
    .x((d: any) => d.x)
    .y((d: any) => d.y)([
        { x: 0, y: 0 },
        { x: width, y: 0 }
...
const tab = shape.line<{x: number, y: number}>()
    .x((d: any) => d.x)
    .y((d: any) => d.y).curve(shape.curveBasis)([
        { x: width, y: 0 },
        { x: width + 20, y: 0 },
        { x: width + 30, y: 10 },
        { x: width + 40, y: 50 },
...
const right = shape.line<{x: number, y: number}>()
    .x((d: any) => d.x)
    .y((d: any) => d.y)([
        { x: width + tabWidth, y: 0 },
        { x: width * 2.5, y: 0 },
...
const Tabbar: React.FC = () => {
    ...
    return (
        <View style = {{width, height: 64}}>
            <AnimatedSVG {...{width: width * 2.5, height: 64}} style = {{transform: [{translateX: widthTranslate}]}}>
                <Path {...{d}} fill = "#A06083"/>
            </AnimatedSVG>
