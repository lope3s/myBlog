...
likeRoute.get(
    ...
    async (
        ...
        try {
            ...
            const commentLike = await db.collection("likes").findOne({
            ...
            if (commentLike) {
                await db
                    .collection("likes")
                    .findOneAndDelete({ _id: commentLike._id });
                await db.collection("comments").findOneAndUpdate(
            ...
            const insertedCommentLike = await db
                .collection("likes")
                .insertOne(like);
            ...
            if (insertedCommentLike.acknowledged) {
                db.collection("comments").findOneAndUpdate(
                    ...
                    {
                        $addToSet: {
                            likes: insertedCommentLike.insertedId.toString() as any,
                        },
                    }
                );
                return res
                    .status(201)
                    .send({ message: "Like adicionado" })
                    .end();
            }

            return res
                .status(500)
                .send({ message: "Algo de errado aconteceu" })
                .end();
        } catch (error) {
            console.log(error);
...
likeRoute.get(
    ...
    async (
        ...
        try {
            ...
            const postLike = await db.collection("likes").findOne({
                userId: new ObjectId(_id),
            ...
            if (postLike) {
                await db
                    .collection("likes")
                    .findOneAndDelete({ _id: postLike._id });
            ...
            const insertedpostLike = await db
                .collection("likes")
                .insertOne(like);
